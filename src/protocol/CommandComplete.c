...
static POOL_STATUS handle_mismatch_tuples(POOL_CONNECTION * frontend, ChildClusterConnection * backend, char *packet, int packetlen, bool command_complete);
...
POOL_STATUS
CommandComplete(POOL_CONNECTION * frontend, ChildClusterConnection * backend, bool command_complete)
{
	int			len,
				len1;
	char	   *p,
 ...
 */
void
handle_query_context(ChildClusterConnection * backend)
{
	POOL_SESSION_CONTEXT *session_context;
	Node	   *node;
