...
extern void handle_query_context(ChildClusterConnection * backend);;

extern void pool_emit_log_for_message_length_diff(int *length_array, char *name);

extern void per_node_statement_notice(ChildClusterConnection * backend, int node_id, char *query);
...
extern bool is_strict_query(Node *node);	/* returns non 0 if this is strict
											 * query */
extern int	need_insert_lock(ChildClusterConnection * backend, char *query, Node *node);
extern POOL_STATUS insert_lock(POOL_CONNECTION * frontend, ChildClusterConnection * backend, char *query, InsertStmt *node, int lock_kind);
...
extern int	check_copy_from_stdin(Node *node);	/* returns non 0 if this is a
												 * COPY FROM STDIN */
extern void query_ps_status(char *query, ChildClusterConnection * backend);	/* show ps status */
extern POOL_STATUS start_internal_transaction(POOL_CONNECTION * frontend, ChildClusterConnection * backend, Node *node);
extern POOL_STATUS end_internal_transaction(POOL_CONNECTION * frontend, ChildClusterConnection * backend);
...
extern bool is_partition_table(ChildClusterConnection * backend, Node *node);
extern POOL_STATUS pool_discard_packet(ChildClusterConnection * cp);
...
extern POOL_STATUS send_extended_protocol_message(ChildClusterConnection * backend,
												  int node_id, char *kind,
												  int len, char *string);

extern int	synchronize(POOL_CONNECTION * cp);
extern void read_kind_from_backend(POOL_CONNECTION * frontend, ChildClusterConnection * backend, char *decided_kind);
extern void read_kind_from_one_backend(POOL_CONNECTION * frontend, ChildClusterConnection * backend, char *kind, int node);
extern void do_error_command(POOL_CONNECTION * backend, int major);
extern void raise_intentional_error_if_need(ChildClusterConnection * backend);

extern void pool_at_command_success(POOL_CONNECTION * frontend, ChildClusterConnection * backend);

/*
 * modules defined in CommandComplete.c
 */
extern POOL_STATUS CommandComplete(POOL_CONNECTION * frontend, ChildClusterConnection * backend, bool command_complete);

extern int	pool_read_message_length(ChildClusterConnection * cp);
extern int *pool_read_message_length2(ChildClusterConnection * cp);
extern signed char pool_read_kind(ChildClusterConnection * cp);
extern int	pool_read_int(ChildClusterConnection * cp);
...
extern char per_node_error_log(ChildClusterConnection * backend, int node_id,
